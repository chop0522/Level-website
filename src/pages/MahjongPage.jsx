  ...
  useEffect(() => {
    async function fetchRank() {
      try {
        setLoading(true)
        setError('')
        const month = tab === 0 ? '' : `?month=${dayjs().subtract(1, 'month').format('YYYY-MM')}`
    ...
    <Box sx={{ p: 3 }}>
      <Helmet>
        ...
        <meta
          ...
          content="今月と先月の月間麻雀ランキング。対局登録のテストは集計対象外です。"
        />
      ...
      {user && user.role === 'admin' && (
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1 }}>
          <IconButton size="small" onClick={() => setFormOpen(true)} title="対局を追加">
            <AddIcon />
          </IconButton>
              ...
              {rows.map((r, i) => {
                ...
                return (
                  <TableRow
                    key={r.id}
                    sx={{
                      bgcolor: user && r.id === user.id ? 'rgba(255,215,0,0.15)' : undefined,
                    }}
                  >
